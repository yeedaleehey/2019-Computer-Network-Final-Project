...
int receive_packet(Packet *packet){
    int packet_len = recvfrom(sockfd, buffer, BufferSize, 0, (struct sockaddr *)&server_info, &addrlen);
...
int receiving_file(char *filename){
    ...
    while(1){
        ...
        if(data_len <= 0) break;
        fwrite(data_packet.data, sizeof(char), data_len, fp);
        ...
        ack_packet.ack_num = data_packet.seq_num + data_len;
